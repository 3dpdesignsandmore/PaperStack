...
**Absent — still to come:** annotation canvas (Phase 6) and store prep (Phase 8) — OCR shipped 2026-09-16 (see §11 Phase 5) minus the invisible-text export layer, which lands with the PDF path when the searchable-text job is wired into `export-document.ts`. `recipients.last_used_at` exists in schema v3 awaiting a share-flow integration the OS share sheet cannot provide on its own — a real one will likely need a share extension or explicit in-app send flow, not `Sharing.shareAsync`.
...
| `react-native-nitro-ocr` | OCR | ~~or `@react-native-ml-kit/text-recognition`~~ **Chosen 2026-09-16:** nitro-ocr keeps the Vision(iOS)/ML Kit(Android) split — the alternative is Android-only, which halves the feature on iOS — and its boxes arrive normalized 0–1 top-left on both platforms. Ships an Expo config plugin (auto-registered in `app.json`; injects ML Kit Latin dep on Android) and peers on `react-native-nitro-modules`. Beta-tagged (0.1.0-beta.0); the cost of choosing it over stability. |
...
**Job 1 — searchable text layer.** ~~After OCR, `drawText` every recognized block at its normalized position using PDF text render mode 3 (invisible).~~ **Shipped in data, pending in export (2026-09-16):** recognition now persists blocks as normalized JSON (`page_ocr.blocks_json`), so the geometry Job 1 needs is already stored per page — what remains is wiring those blocks into `export-document.ts`'s page writes. The glyphs are not painted, but readers can select and search them. This is how every "searchable PDF" works.
...
**Job 2 — field extraction.** ~~On-device ML Kit and Vision return raw text and bounding boxes, not structured fields. The parsing is a heuristic layer you write:~~ **Shipped 2026-09-16** (`src/lib/ocr/extract-receipt.ts`, pure, 19 unit tests; the heuristics below are implemented as written):
...
| **5. OCR** | Library spike, text layer, extraction heuristics, correction UI | ✅ shipped 2026-09-16 (schema v5; engine in the next EAS build). Engine: `react-native-nitro-ocr` (§4 choice). `src/lib/ocr/`: `recognize.ts` (downscale → engine, dynamic import with a catchable stale-binary error — the old pre-flight `requireOptionalNativeModule` gate could never pass, see ocr_issue.md), `extract-receipt.ts` (pure heuristics, 19 unit tests: last total-labeled money wins, top-third date preference, largest-top-third-block merchant), `pipeline.ts` (`scheduleOcr` fires after persist, per-page log-and-continue, respects the `ocr_enabled` setting — a real Settings switch since 2026-09-17). Detail screen shows "Reading document…" then editable merchant/date/total whose corrections set `userEdited` — the never-clobber guard (`saveReceiptData(respectUserEdits)`), plus a manual **Read text** button (the only path pre-OCR documents get OCR); a re-run refreshes `page_ocr` but leaves corrected fields alone. Searchable-PDF export is wired (invisible layer in both document and stacked exports, `pdf/invisible-text.ts`). **Still open:** the engine has never completed a real `recognize()` on device — first EAS preview build after 2026-09-17 is the first genuine run; verify engine bounding boxes are normalized 0..1 and not pixels (ML Kit's Android APIs report pixels; a mismatch silently misplaces the invisible text layer), then remove the `ocr-step` breadcrumbs; receipt fields read the document's first page only — multi-page receipts and a document-vs-receipt kind split are future decisions. Also note the dev client reloads the JS context ~5 s after returning from the ML Kit scanner (same PID, no crash — destroys in-flight async work; documented in ocr_issue.md, dev-only). | 5–7 days |
| **6. Annotation** | Skia canvas; text, highlight, redaction; normalized persist/restore; render into PDF | 5–7 days |
| **7. Sharing polish** | Saved recipients, filename templates | ✅ shipped 2026-09-16, extended same day — recipients are one record per person with two email slots (schema v4: `email_2`, the home + work case; the primary `email` stays the default send target, `emailSlot` in SendOptions picks between them — the send sheet asks when both are set), phone, and merge support; a post-export send sheet (`send-sheet.tsx`) offers one-tap sends per channel plus quick-save of new recipients, with the one-record merge (`recipient-merge.ts`, unit-tested) filling the empty channel field on an existing record instead of duplicating it — an email that doesn't match either slot on a same-label record fills the second slot (home + work) rather than creating a new person. Settings manages the full recipient lifecycle: list (most recently used first, "email · phone" via a shared `channelSummary`), edit (`RecipientFormDialog` doubles as add/edit), pairwise merge of accidental duplicates (`mergeTwoRecords`: kept id survives, unset fields fill from the dropped record, digit-equivalent phones don't conflict, field collisions are confirmed-before-commit), and delete with confirm. Email sends via `expo-mail-composer` (attachment works); text sends via `sms:` link (number filled; iOS attaches nothing via that route — the user sends the file from the OS sheet). Also: full backup (zip of DB + scans + exports via `PRAGMA wal_checkpoint`, `backup.ts`) and a bounded on-device diagnostic log exportable from Settings (`debug-log.ts` — opt-in, no identifiers, no auto-upload; extended 2026-09-17: write-through JSONL disk mirror under documents/debug/ that survives restarts and JS-context reloads, an in-app viewer at `/log`, and — approved, in progress — a **detailed-logging mode**: new `debug_log_until` setting stores a wall-clock deadline rather than a boolean, `logDetail()` is gated on `now < deadline`, the buffer bound rises 500 → 2000 while active, and the window auto-expires after 30 minutes per enable so it cannot be silently extended by a reload or restart; disk side gains size control + rotation — live file `log.jsonl` rotates to `log.1`…`log.5` at 1 MB, 5 files / ~6 MB cap, `clear()` wipes rotated files too, and the export concatenates rotated-then-live so one share carries the whole debug session; the temporary `ocr-step` breadcrumbs become detail-gated, with lean new detail lines for OCR per-page durations and persist/export timings). Note `expo-mail-composer` is a native module — first EAS build after this needs to include it; until that build reaches the device it is imported lazily on the email path only (same pattern as `scanner.ts`), so binaries that predate it keep text sends, the share sheet, and every route alive, with email attempts surfacing a "needs a newer build" notice. | 2 days |
