...
| **5. OCR** | Library spike, text layer, extraction heuristics, correction UI | ✅ shipped 2026-09-16 (schema v5; engine in the next EAS build). Engine: `react-native-nitro-ocr` (§4 choice). `src/lib/ocr/`: `recognize.ts` (downscale → engine, native-gated dynamic import so pre-engine binaries degrade gracefully), `extract-receipt.ts` (pure heuristics, 19 unit tests: last total-labeled money wins, top-third date preference, largest-top-third-block merchant), `pipeline.ts` (`scheduleOcr` fires after persist, per-page log-and-continue, respects the `ocr_enabled` setting). Detail screen shows "Reading document…" then editable merchant/date/total whose corrections set `userEdited` — the never-clobber guard (`saveReceiptData(respectUserEdits)`); a re-run refreshes `page_ocr` but leaves corrected fields alone. **Still open:** wire the invisible text layer (Job 1) into PDF export using the §6 spike mechanics; expose the OCR toggle in Settings (the pipeline reads the key, the UI row still says "Coming soon"); receipt fields read the document's first page only — multi-page receipts and a document-vs-receipt kind split are future decisions. | 5–7 days |
